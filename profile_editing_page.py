...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      ...
      try {
        ...
        const response = await fetch(`**replace this with a dollar sign**{process.env.NEXT_PUBLIC_API_BASE_URL}/user/**replace this with a dollar sign**{userId}`);



        if (!response.ok) {

          throw new Error(`Error: **replace this with a dollar sign**{response.status} **replace this with a dollar sign**{response.statusText}`);
  ...
  const handleSave = async (updatedUser) => {
    ...
    try {
      ...
      const response = await fetch(`**replace this with a dollar sign**{process.env.NEXT_PUBLIC_API_BASE_URL}/user/**replace this with a dollar sign**{displayId}`, {
